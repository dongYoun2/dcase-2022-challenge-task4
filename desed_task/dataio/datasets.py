...
class StronglyAnnotatedSet(Dataset):
    def __init__(
        ...
        assert embedding_type in [
            ...
            "frame",
            None,
        ], "embedding type are either frame or global or None, got {}".format(embedding_type)
...
class WeakSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        mixture, _, _, padded_indx = read_audio(c_ex["mixture"], self.multisrc, self.random_channel, self.pad_to)
...
class UnlabeledSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        mixture, _, _, padded_indx = read_audio(c_ex, self.multisrc, self.random_channel, self.pad_to)
