...
def extract(batch_size, folder, dset_name, torch_dset, embedding_model, use_gpu=True):
    ...
    global_embeddings = f.create_dataset("global_embeddings", (len(torch_dset),), dtype=dt)
    ...
    dloader = torch.utils.data.DataLoader(torch_dset, batch_size=batch_size, drop_last=False)
    global_indx = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf_file", default="./confs/default.yaml", help="The configuration file with all the experiment parameters."
    )
    parser.add_argument(
        "--pretrained_model", default="panns", help="The pretrained model to use," "choose between panns and ast"
    )
    parser.add_argument("--use_gpu", default="1", help="0 does not use GPU, 1 use GPU")
    parser.add_argument(
        "--batch_size", default="8", help="Batch size for model inference, used to speed up the embedding extraction."
    )
    ...
    if args.pretrained_model == "ast":
        ...
        class ASTFeatsExtraction:
            ...
            def __init__(self, audioset_mean=-4.2677393, audioset_std=4.5689974, target_length=1024):
    ...
    synth_set = WavDataset(config["data"]["synth_folder"], feats_pipeline=feature_extraction)
    ...
    weak_set = WavDataset(config["data"]["weak_folder"], feats_pipeline=feature_extraction)

    unlabeled_set = WavDataset(config["data"]["unlabeled_folder"], feats_pipeline=feature_extraction)

    synth_df_val = pd.read_csv(config["data"]["synth_val_tsv"], sep="\t")
    synth_val = WavDataset(config["data"]["synth_val_folder"], feats_pipeline=feature_extraction)

    weak_val = WavDataset(config["data"]["weak_folder"], feats_pipeline=feature_extraction)

    devtest_dataset = WavDataset(config["data"]["test_folder"], feats_pipeline=feature_extraction)
    """
